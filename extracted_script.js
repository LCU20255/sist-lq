    ...
    document.addEventListener('DOMContentLoaded', () => {
        const astridGreet = {{ astrid_greet | tojson | default('false') }};
        if(astridGreet && typeof hablarAstrid === 'function') {
            const nombre = '{{ session.get("user_nombre", "administrador") }}';
            setTimeout(() => {
                hablarAstrid("Bienvenido " + nombre + ", soy Astrid. Todos los sistemas están en línea y a tu disposición.");
            }, 1000); // 1 sec delay to ensure voices are loaded
        }
    });
\n
    ...
    try {
        ...
        if(data.success) {
            ...
            if(ordenes.length === 0) {
                ...
                if (ordenes[0]) document.getElementById('perfil_ultima_fecha').innerText = ordenes[0].fecha;
            }
